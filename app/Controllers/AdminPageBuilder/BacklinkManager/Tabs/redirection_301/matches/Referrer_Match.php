...
class Referrer_Match extends Aios_Match{
	public $referrer;
	public $regex;

	function name() {
		return __( 'URL and referrer', SR_TEXTDOMAIN );
	}
	...
	function show() {
		...
		?>
		<tr>
			<th width="100"><?php _e( 'Referrer', SR_TEXTDOMAIN ); ?>:</th>
			<td valign="top">
				<input style="width: 85%" type="text" name="referrer" value="<?php echo esc_attr( $this->referrer ); ?>"/>
				<label><?php _e( 'Regex', SR_TEXTDOMAIN ); ?>: <input type="checkbox" name="regex" <?php if ( $this->regex === true ) echo ' checked="checked"' ?>/></label>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<th width="100" valign="top">
				...
				<a target="_blank" href="<?php echo $this->url_notfrom ?>"><?php _e( 'Not matched', SR_TEXTDOMAIN ); ?>:</a>
				<?php else : ?>
