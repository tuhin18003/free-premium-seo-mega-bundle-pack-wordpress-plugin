...
class VisitorAllClickedLink {
    ...
    public function load_page_builder(){
       ...
       if( isset( $_GET['date_range']) && !empty($_GET['date_range'])){
            ...
            $this->first_day = date('Y-m-d', strtotime($date_range[0])) .' 11:59:00';
            $this->last_day = date('Y-m-d', strtotime($date_range[1])) .' 11:59:00';
       }else{
           $this->first_day = date('Y-m-d', strtotime('2 months ago')) .' 11:59:00';
           $this->last_day = date('Y-m-d', strtotime("+1 day")) .' 11:59:00';
       }
       
       $data_item = CsQuery::Cs_Get_Results(array(
           'select' => '*',
           'from' => $wpdb->prefix . 'aios_visitor_click_tracker',
           'where' => "visited_on between '{$this->first_day}' and '{$this->last_day}'",
           'order_by' => 'id desc'
       ));
       ...
       $data = array_merge($CommonText->form_element('aios-visitor-clicked-link'), array(
           ...
           'page_title' =>  __( 'Visitor Clicked Link List', SR_TEXTDOMAIN ),
           'page_subtitle' =>  __( 'Monitor & Manage All Links Clicked By Your Visitor', SR_TEXTDOMAIN ),
           'tbl_headers' => array(
                   __( 'Link', SR_TEXTDOMAIN ),
                   __( 'Link Keyword', SR_TEXTDOMAIN ),
                   __( 'Url Referrer', SR_TEXTDOMAIN ),
                   __( 'User Ref.', SR_TEXTDOMAIN ),
           ...
           'current_url' => admin_url('admin.php?page=cs-backlink-manager&tab=VisitorAllClickedLink'),
       ...
       return  view( '@CsSeoMegaBundlePack/backlinkManager/tabs/visitorClickTracking/VisitorAllClickedLink.twig', $data );
    }
