...
class ManageBacklinkCost {
    ...
    public function load_page_builder(){
       ...
       $items = CsQuery::Cs_Get_results(array(
            ...
            'where' => "c.type = 2 and c.created_on between '{$this->first_day}' and '{$this->last_day}' order by c.created_on desc"
        ));
        $cost = CsQuery::Cs_Sum(array(
            'table' => 'aios_blmanager_item_price',
            'where' => "type = 2 and created_on between '{$this->first_day}' and '{$this->last_day}' ",
            'sum_of' => 'price'
        ));
       ...
       $data = array_merge( (new CommonText())->form_element( 'aios-add-blc-cost' ), array(
           ...
           'page_title' =>  __( 'Track Backlink Cost', SR_TEXTDOMAIN ),
           'page_subtitle' =>  __( 'Track Your Backlink Cost', SR_TEXTDOMAIN ),
           'tbl_headers' => array(
                   __( 'Backlink Group', SR_TEXTDOMAIN ),
           ...
           'no_group_found' =>  __( 'Please add backlink group to add cost!', SR_TEXTDOMAIN ),
           ...
           'current_url' => admin_url('admin.php?page=cs-backlink-manager&tab=ManageBacklinkCost'),
           'add_group_url' => admin_url('admin.php?page=cs-backlink-manager&tab=ManageBacklinkGroups'),
       ...
       return  view( '@CsSeoMegaBundlePack/backlinkManager/tabs/backlinkChecker/ManageBacklinkCost.twig', $data );
    }
