...
class YandexWebmasterVerification {
    /**
     * @var type Obj
     */
    protected $http;
    ...
    public function load_page_builder( $common_text ){
        ...
        // check for token
        $data = array_merge( CommonText::form_element( 'save_yandex_meta_tag', 'yandex-webmaster-verification' ), array(
           'CommonText' => $common_text,
           'page_title' =>  __( 'Yandex Site Verification', SR_TEXTDOMAIN ),
           'page_subtitle' =>  __( 'This section lets you automatically verifiy ownership of websites or domains with Yandex Webmaster Tool.', SR_TEXTDOMAIN ),
           ...
           'des_meta_tag_1' => __( "Enter here your meta tag content from Yandex webmaster. Example: eaa44875f64e3f872875052e1fd8411d", SR_TEXTDOMAIN ),
           'verify_meta_tag' => isset($this->options->webmaster_verify_code->yandex) ? $this->options->webmaster_verify_code->yandex : '',
           ...
           'app_create_hints' => array(
               sprintf(__( '%1$s Sign up / Login %2$s here %1$s%3$s%2$s to submit your website.', SR_TEXTDOMAIN ), '<a href="https://webmaster.yandex.com/" target="_blank">', '</a>' , 'https://webmaster.yandex.com/' ),
               __( 'After submitting your website, Verify now\'. Click verify now.', SR_TEXTDOMAIN ),
               ...
               __( 'Go Back to the Yandex webmaster tool page and click Verify button bellow of the page. Your website will be verified successfully.', SR_TEXTDOMAIN )
           )
       ));
       add_action('admin_footer', [$this, '_addFooter_script']);
       return  view( '@CsSeoMegaBundlePack/onPageOptimization/tabs/SearchEngine/YandexWebmasterVerification.twig', $data );
    }
    ...
    private function verification_status(){
        $notificatoin = '';
        if( isset($this->options->webmaster_verify_status->yandex) && !empty($this->options->webmaster_verify_status->yandex) && $this->options->webmaster_verify_status->yandex == 'success'){
            $notificatoin =  __( '%s Successfully Verified! %s your website has been successfully verifed by Yandex webmaster tool.', SR_TEXTDOMAIN );
