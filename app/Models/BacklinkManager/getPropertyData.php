...
class getPropertyData {
     ...
     * @return boolean | obj
     */
    public function getProperty(  $condition = array()  ){
        global $wpdb;
        ...
        if( isset( $condition['compare_by_date'] ) && $condition['compare_by_date'] === true ) {
            ...
            $where = " where p.id in (".$condition['p_id'].") "  . ' and p.item_type = 1 ';
        }
       
        if( isset( $condition['group_id'] ) && ! empty( $condition['group_id'] ) ) {
            if(empty( $where) ){
                $where = ' where p.item_group_id = ' . $condition['group_id'] . ' and p.item_type = 1 ';
            }else{
                $where = $where . ' and p.item_group_id = ' . $condition['group_id'] . ' and p.item_type = 1 ';
            }
        }
        $query = $wpdb->get_results( "SELECT *,p.id as p_id FROM `{$wpdb->prefix}aios_blmanager_items` as p LEFT JOIN {$subQuery} AS ps ON p.id = ps.item_id {$date_range} {$where} {$group_by} {$order}" );
        if( $query ){
            return $query;
        }
        return false;
